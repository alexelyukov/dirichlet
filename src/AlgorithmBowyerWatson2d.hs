...
isPointInCircle (Point2d x y) t =
  let Circle (Point2d centerX centerY) radius = getCircleAroundTriangle t
...
checkTriangulation :: [Triangle] -> [Point2d] -> Bool
checkTriangulation ts ps =
  let isPointBelongTriangle p0 (Triangle p1 p2 p3) = p0 ~= p1 || p0 ~= p2 || p0 ~= p3
  in and [isPointBelongTriangle p t || not (isPointInCircle p t) | t <- ts, p <- ps]

getCircleAroundTriangle :: Triangle -> Circle
getCircleAroundTriangle (Triangle (Point2d ax ay) (Point2d bx by) (Point2d cx cy)) =
  ...
  in Circle (Point2d centerX centerY) radius